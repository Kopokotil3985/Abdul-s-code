define e = Character('Женщина', color="#c94f39")
...
define i = Character('Ильинична', color = "#388332")
define sosed = Character('Сосед')
define gg = Character('???', color = "#8157cf")
...
label start:
    ...
    "Только Бог знает."
    show egorich_happy at left with dissolve
    show borka_happy at right with dissolve
    B "Привет, Егорыч!"
    E "Привет, Борька!"
    ...
    show kirilovna_angry at left2 with dissolve
    ...
    show anastsaia_normal at left with dissolve
    show egorich_normal at right with dissolve
    ...
    E "Моя дорогуша, а не вы ли курите на лесенках?"
    E "Выхожу я значит в 6.52 выбросить мусор и, решив пройти по ступенькам этажа так 3, размять, как говорится свои старые ноги,{w} бычок,{w} потом ещё один, 3 и если выражаться точным алгебраическим языком их было 21 штука."
    E "Вы курите?"
    ...
    show anastsaia_normal at left with dissolve
    show igorevna_normal at left with dissolve
    i "Давайте, Анастасия Игоревна, помогите пожалуйста, квартиру Олега Егоровича ограбили!"
    ...
    scene egorich_home_crashed with dissolve
    "Войдя в квартиру без двери, та лежала на полу, я увидела разбитый шифоньер, достояние Егорыча"
    "Открытое окно,"
    "Егоровича, рассказывающего что-то милициоенру"
    "И приготовленный ужин."
    scene black with fade
    "Затем 2 часа давала показания."
    "Скукотища!"
    "Как можно отменить пытки и оставить нужные вопросы полицейских:"
    ...
    "И конечно камеры, очи правительства! От них не уйти."
    "Но он замел следы…"
    window hide
    scene chapter1 with fade
    pause
    scene black with fade
    nvl "В оригинальной версии написана назывными нераспространенными предложениями, но для облегчения читателя будет изменена, но в простой манере."
    nvl "Все будет происходить в голове нервно спящего человека!"
    nvl clear
    "Детство моё проходило на морском побережье в Астраханской области."
    "Отец - рыбак много пил."
    "А мать боролась с ним и рано умерла,"
    menu:
        "Но была злой, любви я от нее не чувствовал.":
            $ end += 1
        "Она одна любила меня.":
            "Это отец убил маму ночью."
    ...
    "Грабеж."
    "Побег."
    "Пришлось бежать в лес, вышел на остановке 666 автобуса, загородный какой-то."
    "Статуя Зевса."
    "Свет."
    "Ночь."
    "Свет."
